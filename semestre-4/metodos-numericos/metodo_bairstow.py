...
class MetodoBairstow():
  ...
  def execute(vetor_coeficientes,r,s,grau,epsilon):
    ...
    while grau >= 3:
        ...
        while igrau >0:
            ...
            if igrau == grau-1:
                ...
                bi[igrau]=vetor_coeficientes[igrau]+r*bi[igrau+1]+s*bi[igrau+2]
        igrau=grau
        ci[igrau]=bi[igrau]
        while igrau>1:
            igrau=igrau-1
        ...
        while igrau >=0:
            ...
            igrau=igrau-1
    if grau == 2:
        delta = vetor_coeficientes[1]**2-4*vetor_coeficientes[2]*vetor_coeficientes[0]
        if delta >= 0:
            raizes[id_raiz]=(-1*vetor_coeficientes[1]+math.sqrt(delta))/(2*vetor_coeficientes[2])
            print("Raiz = ", raizes[id_raiz])
            id_raiz=id_raiz+1
            raizes[id_raiz]=(-1*vetor_coeficientes[1]-math.sqrt(delta))/(2*vetor_coeficientes[2])
            print("Raiz = ", raizes[id_raiz])
            ...
            raizes[id_raiz]=(-1*vetor_coeficientes[1]+complex(0,math.sqrt(-1*delta)))/(2*vetor_coeficientes[2])
            print("Raiz = ", raizes[id_raiz])
            id_raiz=id_raiz+1
            raizes[id_raiz]=(-1*vetor_coeficientes[1]-complex(0,math.sqrt(-1*delta)))/(2*vetor_coeficientes[2])
            print("Raiz = ", raizes[id_raiz])
